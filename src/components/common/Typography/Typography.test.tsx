...
import { TypographyVariant, ValueFormat } from './types'
...
describe('Typography Component', () => {
  describe('Rendering', () => {
    ...
    it('should render with custom testID', () => {
      const { getByTestId } = render(<Typography testID="custom-text">Test Text</Typography>)
  ...
  describe('Variants', () => {
    ...
    variants.forEach(variant => {
      it(`should apply correct style for ${variant} variant`, () => {
        const { getByText } = render(<Typography variant={variant}>Test Text</Typography>)
  ...
  describe('Formatting', () => {
    it('should format currency values correctly', () => {
      const value = 1000
      const { getByText } = render(<Typography format={ValueFormat.CURRENCY}>{value}</Typography>)
    ...
    it('should not format text when format is TEXT', () => {
      ...
      const { getByText } = render(<Typography format={ValueFormat.TEXT}>{text}</Typography>)
  ...
  describe('Styling', () => {
    it('should apply custom color', () => {
      ...
      const { getByText } = render(<Typography color={customColor}>Test Text</Typography>)
    ...
    it('should apply custom margins and padding', () => {
      ...
      const { getByText } = render(
        <Typography marginBottom={marginBottom} paddingHorizontal={paddingHorizontal}>
          Test Text
        </Typography>
      )
    ...
    it('should apply custom text alignment', () => {
      const { getByText } = render(<Typography align="center">Test Text</Typography>)
    ...
    it('should apply custom style prop', () => {
      ...
      const { getByText } = render(<Typography style={customStyle}>Test Text</Typography>)
